...
int main(int argc, char *argv[]) {
    /**csv reader**/
    prm_localization::CSV_reader csvReader(
            "/home/vickylzy/workspaceROS/MAP_BAG/wuhan/"
            "Transform_map_GNSS_3dbag_ls700b_1204_1/"
            "3dbag_ls700b_1204_1_M_map_GNSS.csv");
    ...
    //    nh.advertise<nav_msgs::Odometry>("/stamp",5);
    //
    ...
    //    sham.push_back(5);
    //    size_t b = sham.size();
    //    for (boost::circular_buffer<int>::const_iterator i =
    //    sham.end()-1;i!=sham.begin();i--){
    //        cout<<*i<<endl;
    //    }
    ...
    //    sham.push_back(odom);
    //    size_t b = sham.size();
    //    for (boost::circular_buffer<nav_msgs::OdometryConstPtr>::const_iterator
    //    i = sham.end()-1;i!=sham.begin();i--){
    //        nav_msgs::Odometry odometry = **i;
    ////        NODELET_INFO("check a odom_msg");
    //        if(odometry.header.stamp <= odometry.header.stamp){
    //            Quaternionf
    //            q(odometry.pose.pose.orientation.w,odometry.pose.pose.orientation.x,odometry.pose.pose.orientation.y,odometry.pose.pose.orientation.z);
    //            Matrix4f odom_pose;
    ...
    //     // initialize DriveWorks PointCloudAccumulator module which will
    //     accumulate individual lidar packets to a point cloud
    ...
    //     dwPointCloudAccumulator_initialize(&gAccumulator, &accumParams,
    //     &gLidarProperties, gContext);
    ...
    //     // initialize DriveWorks PointCloudStitcher which will combine multiple
    //     point clouds into one dwPointCloudStitcher_initialize(&gStitcher,
    //     gContext);

    //     // allocate buffer to generate depthmap representation from lidar
    //     points gDepthMap.type     = DW_MEMORY_TYPE_CUDA; gDepthMap.capacity =
    //     getDepthmapWidth() * getDepthmapHeight();
    //     dwPointCloud_createBuffer(&gDepthMap);

    //     // create RangeImageCreator which will generate 2.5D depthmap as well
    //     as range image representation from unorganized accumulated points
    //     dwPointCloudRangeImageCreator_initialize(&gRangeImageCreator,
    //     &gRangeImageCreatorProperties, gContext);

    //     // initialize DriveWorks ICP module
    //     dwPointCloudICPParams icpParams{};
    //     icpParams.icpType  = DW_POINT_CLOUD_ICP_TYPE_DEPTH_MAP; // optimized
    //     ICP for 2.5D depthmap representation of a point cloud
    //     dwPointCloudICP_initialize(&gICP, &icpParams, gContext);
    // }
    /**learn copy_if back insert**/
    //        std::string globalmap_pcd =
    //        "/home/vickylzy/WorkSPacesROS/catkin_ws/src/prm_localization/data/shunYuFactory.pcd";//private_nh.param<std::string>("globalmap_pcd",
    //        ""); pcl::PointCloud<PointT>::Ptr cloud (new
    //        pcl::PointCloud<PointT>); pcl::io::loadPCDFile(globalmap_pcd,
    //        *cloud);
    //
    //        pcl::PointCloud<PointT>::Ptr filtered(new
    //        pcl::PointCloud<PointT>()); filtered->reserve(cloud->size());
    //
    ...
    //    std::string globalmap_pcd =
    //    "/home/vickylzy/WorkSPacesROS/catkin_ws/src/prm_localization/data/shunYuFactory.pcd";//private_nh.param<std::string>("globalmap_pcd",
    //    ""); std::string currmap_pcd =
    //    "/home/vickylzy/WorkSPacesROS/catkin_ws/src/prm_localization/data/shunYu1.pcd";//private_nh.param<std::string>("globalmap_pcd",
    //    "");
    //
    //    pcl::PointCloud<pcl::PointXYZRGB>::Ptr full_cloud (new
    //    pcl::PointCloud<pcl::PointXYZRGB>);
    //    pcl::PointCloud<pcl::PointXYZRGB>::Ptr curr_cloud (new
    //    pcl::PointCloud<pcl::PointXYZRGB>);
    //
    ...
    //    pcl::PointCloud<pcl::PointXYZRGB>::Ptr result (new
    //    pcl::PointCloud<pcl::PointXYZRGB>);
    //    /** analyse RANSACOutlierRejectionThreshold and
    //    MaxCorrespondenceDistance**/ for (double i = 0.01; i <0.15 ; i+=0.01) {
    //        icp.setRANSACOutlierRejectionThreshold(i);
    //        icp.setMaxCorrespondenceDistance(i*100);
    //        clock_t start = clock();
    //        icp.align(*result);
    //        clock_t end = clock();
    //        cout<<"registration in "<< (double)(end  - start) / CLOCKS_PER_SEC
    //        << "second" << endl; cout<<"with"<<
    //        "registration.setRANSACOutlierRejectionThreshold() = "<< i <<endl;
    //        cout<<"registration.getFitnessScore() =
    //        "<<icp.getFitnessScore()<<endl; cout<<endl;
    //
    //    }
    ...
    //        clock_t end = clock();
    //        cout<<"registration in "<< (double)(end  - start) / CLOCKS_PER_SEC
    //        << "second" << endl; cout<<"with"<< "TransformationEpsilon() = "<< i
    //        <<endl; cout<<"registration.getFitnessScore() =
    //        "<<icp.getFitnessScore()<<endl; cout<<endl;
    //
    //    }
    ...
    //    }
    //    cout<<"init in "<< (double)(start  - init) / CLOCKS_PER_SEC << "second"
    //    << endl;
    //
    //    pcl::visualization::CloudViewer viewer ("Simple Cloud Viewer");
    //    pcl::PointCloud<pcl::PointXYZRGB>::Ptr show (new
    //    pcl::PointCloud<pcl::PointXYZRGB>); *show = *result+*full_cloud;
