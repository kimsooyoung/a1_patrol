...
<launch>
    ...
    <!-- 매개변수, 지도 이름, 기본값 "map_test" 설정 -->
    <arg name="map_name" default="map_test"/>
    
    <!-- 라이다 구동을 개시하다. -->
    <include file="$(find velodyne_pointcloud)/launch/VLP16_points.launch"/>
    <!-- 위치 측정을 위한 3D 점 구름 일치 알고리즘 시작 -->
...
// 전역의 map 좌표계를 국소 local_map 좌표계로 변환합니다. 
// x, y는 동일하지만 local_map의 z축 좌표는 로봇 개의 z 좌표와 동일합니다.
...
int main(int argc, char *argv[]){
    ...
    while (ros::ok()){
        ...
        // 받은 좌표 변환에 따라 publish할 좌표 변환을 구성합니다.
...
// 이 프로그램은 ros를 통해 순찰 좌표점을 배포하는 데 사용되며 move_base의 actionlib 라이브러리를 사용하므로 단순한 topic 배포 순찰점보다 작업 수행 결과를 알 수 있는 장점이 있습니다.
