...
/**
 * @brief geometry_msgs/Twist 및 기타 여러 topic msg들을 unitree_legged_msgs::HighCmd로 변환
 *
 */
class UnitreeTwistCmd
{
private:
  /// ROS topic Sub/Pub
...
public:
   ...
   */
  UnitreeTwistCmd(ros::NodeHandle *nh);

  /**
   * @brief Position Control Mode라는 가정 하에 twist 타입 데이터를 Subscribe 하여
   * unitree 호환의 제어 데이터로 변환한다.
   * 
   * @param twist geometry_msgs::Twist 타입 데이터
   */
  void twistSubCallback(const geometry_msgs::Twist &twist);
   ...
   */
  void controlModeSubCallback(const std_msgs::UInt8 &msg);

  /**
   * @brief Float32 타입의 topic msg를 Subscribe 하여 로봇의 몸체 높이를 설정한다.
   * default = 0
   * 
   * @param msg std_msgs::Float32 타입 데이터
   */
  void bodyHeightSubCallback(const std_msgs::Float32 &msg);
   ...
   * @param msg std_msgs::Float32 타입 데이터
   */
  void footHeightSubCallback(const std_msgs::Float32 &msg);
